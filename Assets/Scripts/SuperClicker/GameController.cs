...
public class GameController : MonoBehaviour
{
	...
	void Awake()
	{
		s_rewardPanel = FindObjectOfType<RewardPanel>();
	...
	private void GetReward(Reward reward)
	{
		...
		if (reward.RewardType == RewardType.Agent)
		{
			if(reward.Value >= 0 && reward.Value < _agents.Length)
			{
                ...
                if (reward.Value == 1) 
                {
					s_rewardPanel.RewardPrimerBlob();
                }
				if (reward.Value == 2)
				{
					s_rewardPanel.RewardPrimerBebe();
				}
				if (reward.ObjectReward != null)
				{
					...
					if (newAgent != null)
					{
						...
						if (newAgent.destiny == null)
						{
							Debug.LogError("Error: Se ha instanciado un agente sin destino.");
						}
					}
				}
